...
class FoundationPose:
  def __init__(self, model_pts, model_normals, symmetry_tfs=None, mesh=None, scorer:ScorePredictor=None, refiner:PoseRefinePredictor=None, glctx=None, debug=0, debug_dir='/home/bowen/debug/novel_pose_debug/'):
    ...
    self.make_rotation_grid(min_n_views=20, inplane_step=60)
  ...
  def register(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5):
    ...
    if self.glctx is None:
      if glctx is None:
        ...
        self.glctx = glctx
    toc("FoundationPose.register.prepare_glctx", t_glctx)

    t_depth_pre = tic()
    depth = erode_depth(depth, radius=2, device='cuda')
    depth = bilateral_filter_depth(depth, radius=2, device='cuda')
    toc("FoundationPose.register.depth_preprocess", t_depth_pre)
    ...
    valid_num = valid.sum()
    toc(f"FoundationPose.register.valid_check | valid={valid_num}", t_valid)
    if valid_num<4:
      logging.info(f'valid too small, return')
      t_guess_small = tic()
      pose = np.eye(4)
      pose[:3,3] = self.guess_translation(depth=depth, mask=ob_mask, K=K)
      toc("FoundationPose.register.guess_translation_small_valid", t_guess_small)
      toc("FoundationPose.register.total | early_return", t_register_total)
      return pose
    ...
    toc(f"FoundationPose.register.generate_pose_hypo | N={poses.shape[0]}", t_hypo)

    t_guess = tic()
    center = self.guess_translation(depth=depth, mask=ob_mask, K=K)
    toc("FoundationPose.register.guess_translation", t_guess)

    t_pose_to_cuda = tic()
    poses = torch.as_tensor(poses, device='cuda', dtype=torch.float)
    poses[:,:3,3] = torch.as_tensor(center.reshape(1,3), device='cuda')
    toc("FoundationPose.register.pose_to_cuda_and_set_center", t_pose_to_cuda)
    ...
    t_xyz = tic()
    xyz_map = depth2xyzmap(depth, K)
    toc("FoundationPose.register.depth2xyzmap", t_xyz)
    ...
    toc(f"FoundationPose.register.refiner.predict | iteration={iteration}", t_refiner)
    if vis is not None:
      t_save_ref_vis = tic()
      imageio.imwrite(f'{self.debug_dir}/vis_refiner.png', vis)
      toc("FoundationPose.register.save_vis_refiner", t_save_ref_vis)
    ...
    toc("FoundationPose.register.scorer.predict", t_scorer)
    if vis is not None:
      t_save_score_vis = tic()
      imageio.imwrite(f'{self.debug_dir}/vis_score.png', vis)
      toc("FoundationPose.register.save_vis_score", t_save_score_vis)
    ...
    toc("FoundationPose.register.sort_scores", t_sort)
    ...
    out = best_pose.data.cpu().numpy()
    toc("FoundationPose.register.prepare_output", t_best)
    toc("FoundationPose.register.total", t_register_total)
    return out
  ...
  def track_one(self, rgb, depth, K, iteration, extra={}):
    ...
    toc("FoundationPose.track_one.depth_preprocess", t_depth)

    t_xyz = tic()
    xyz_map = depth2xyzmap_batch(depth[None], torch.as_tensor(K, dtype=torch.float, device='cuda')[None], zfar=np.inf)[0]
    toc("FoundationPose.track_one.depth2xyzmap_batch", t_xyz)
    ...
    toc(f"FoundationPose.track_one.refiner.predict | iteration={iteration}", t_refiner)
    if self.debug>=2:
      extra['vis'] = vis
    self.pose_last = pose
    t_output = tic()
    out = (pose@self.get_tf_to_centered_mesh()).data.cpu().numpy().reshape(4,4)
    toc("FoundationPose.track_one.prepare_output", t_output)
    toc("FoundationPose.track_one.total", t_track_total)
    return out
