...
4. 如果 tracking 质量低于阈值：重新对所有 OBJ register()，重新选择模型
...
- 如果每一帧都有 mask，重选模型会更稳。
- 如果只有第一帧 mask，后续重选会用上一帧渲染 mask 作为弱 fallback。
"""
...
def choose_reinit_mask(reader, i, last_render_mask, args):
    mask = get_mask_safe(reader, i)

    if mask is not None:
        print(f"[INFO] frame {i}: 使用当前帧真实 mask 重新选择模型")
        return mask
...
def main():
    ...
    for mesh_file in mesh_files:
        name = Path(mesh_file).parent.name
    ...
    for i in range(len(reader.color_files)):
        ...
        if i == 0 or active is None:
            ...
            try:
                ...
                current_mask = get_mask_safe(reader, i)
            ...
            if tracking_failed:
                print("[RESELECT] tracking 失败，尝试重新对所有 OBJ register()")
                ...
                if reinit_mask is not None:
                    ...
                    selected_records.append({
                        ...
                        "mesh_file": active["mesh_file"],
                        "score": best["score"],
                        "reason": "reselect_after_tracking_failed"
                    })

                    print(f"[ACTIVE] 重选后使用模型: {active['name']}")

                else:
                    print("[WARN] 重新 register 失败：没有可用 mask。继续使用上一模型，但该帧 pose 可能不可靠。")
                    if pose is None:
                        continue
                    ...
                    selected_records.append({
                        ...
                        "reason": "tracking_failed_no_reinit_mask"
                    })
