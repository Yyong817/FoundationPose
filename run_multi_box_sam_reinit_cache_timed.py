...
from estimater_timed import *
from datareader import *
from Utils import *
from register_render_cache_utils_timed import load_caches_for_candidates, register_with_cached_topk
...
def load_mesh_safe(mesh_file):
    ...
    toc(f"load_mesh_safe | {Path(mesh_file).name}", t_func)
    return mesh
...
def build_estimator(mesh_file, scorer, refiner, glctx, debug_dir, debug):
    ...
    toc(f"build_estimator.load_mesh | {get_object_name_from_mesh_path(mesh_file)}", t_load)
    ...
    toc(f"build_estimator.FoundationPose_init | {get_object_name_from_mesh_path(mesh_file)}", t_est)

    t_bbox = tic()
    to_origin, extents = trimesh.bounds.oriented_bounds(mesh)
    bbox = np.stack([-extents / 2, extents / 2], axis=0).reshape(2, 3)
    toc(f"build_estimator.oriented_bbox | {get_object_name_from_mesh_path(mesh_file)}", t_bbox)
    ...
    toc(f"build_estimator.total | {get_object_name_from_mesh_path(mesh_file)}", t_func)
    return out
...
def score_candidate(candidate, rgb, depth, K, ob_mask=None,
    ...
    toc(f"score_candidate.render_candidate | {candidate.get('name', 'unknown')}", t_render)

    if render_rgb is None:
        toc(f"score_candidate.total | {candidate.get('name', 'unknown')} | failed_no_pose", t_score_total)
        return -1.0, {
            "ok": False,
    ...
    toc(f"score_candidate.metric_compute | {candidate.get('name', 'unknown')}", t_metric)
    toc(f"score_candidate.total | {candidate.get('name', 'unknown')}", t_score_total)
...
def select_best_object(candidates, rgb, depth, ob_mask, K, est_refine_iter,
    ...
    for idx, cand in enumerate(candidates):
        ...
        try:
            print("start", getattr(args, "use_render_cache", False))
            t_register = tic()
            if getattr(args, "use_render_cache", False):
                pose, cache_records = register_with_cached_topk(
                    candidate=cand,
                    rgb=rgb,
                    depth=depth,
                    ob_mask=ob_mask,
            ...
            toc(f"select_best_object.register_total | {name}", t_register)
            ...
            toc(f"select_best_object.score_total | {name}", t_score)
            ...
            toc(f"select_best_object.candidate_total | {name}", t_candidate_total)

        except Exception as e:
            toc(f"select_best_object.candidate_total | {name} | failed", t_candidate_total)
    ...
    toc(f"select_best_object.total | frame={frame_id}", t_select_total)
    return best, results
...
def get_mask_safe(reader, i, mask_dir=None, frame_id=None, target_shape=None):
    ...
    if mask_path is not None:
        try:
            ...
            toc(f"get_mask_safe.external_mask | frame={frame_id}", t_mask_total)
    ...
    try:
        ...
        toc(f"get_mask_safe.reader_mask | frame={frame_id}", t_mask_total)
        return mask.astype(bool)
    except Exception:
        toc(f"get_mask_safe.none | frame={frame_id}", t_mask_total)
        return None
...
def choose_reinit_mask(reader, i, last_render_mask, args, frame_id=None, target_shape=None):
    ...
    if mask is not None:
        ...
        toc(f"choose_reinit_mask.current_mask | frame={frame_id}", t_choose_mask)
        return mask
    ...
    if args.allow_render_mask_fallback and last_render_mask is not None:
        ...
        toc(f"choose_reinit_mask.render_fallback | frame={frame_id}", t_choose_mask)
        return last_render_mask.astype(bool)

    print(f"[WARN] frame {i}: 没有可用 mask，无法重新 register")
    toc(f"choose_reinit_mask.none | frame={frame_id}", t_choose_mask)
    return None
...
def register_active_object(active, rgb, depth, ob_mask, K, args, frame_id):
    ...
    toc(f"register_active_object.register_total | {active['name']} | frame={frame_id}", t_active_register)
    ...
    toc(f"register_active_object.score_total | {active['name']} | frame={frame_id}", t_active_score)
    ...
    toc(f"register_active_object.total | {active['name']} | frame={frame_id}", t_active_total)
    return out
...
def save_pose(debug_dir, frame_id, pose):
    ...
    toc(f"save_pose | frame={frame_id}", t_save_pose)
...
def save_scores(debug_dir, frame_id, score_records):
    ...
    toc(f"save_scores | frame={frame_id}", t_save_scores)
...
def visualize_and_save(candidate, pose, reader, color, frame_id, debug_dir,
    ...
    toc(f"visualize_and_save.total | frame={frame_id}", t_vis_total)
    return vis, key
...
def main():
    ...
    parser.add_argument("--use_render_cache", action="store_true", default=1,
                        help="使用离线预渲染模板库筛选 top-K 初始旋转，从而减少 register 耗时")

    parser.add_argument("--render_cache_dir", type=str, default="./register_render_cache",
                        help="离线预渲染模板缓存目录")

    parser.add_argument("--cache_topk", type=int, default=10,
                        help="每次 register 前，从预渲染模板中选 top-K 个旋转进入 FoundationPose.register")

    parser.add_argument("--cache_template_size", type=int, default=160,
                        help="预渲染模板尺寸，需和 build_register_render_cache.py 保持一致")

    parser.add_argument("--cache_crop_pad_ratio", type=float, default=1.2,
                        help="当前帧 mask crop 的外扩比例")

    parser.add_argument("--cache_texture_weight", type=float, default=0.85,
                        help="预渲染模板匹配时的 RGB 纹理权重")

    parser.add_argument("--cache_mask_weight", type=float, default=0.15,
                        help="预渲染模板匹配时的 mask IoU 权重")
    ...
    toc("main.setup_logging_debug_dirs", t_setup)

    t_discover = tic()
    mesh_files = discover_mesh_files(args)
    toc("main.discover_mesh_files", t_discover)
    ...
    toc("init.ScorePredictor", t_scorer)

    t_refiner = tic()
    refiner = PoseRefinePredictor()
    toc("init.PoseRefinePredictor", t_refiner)

    t_glctx = tic()
    glctx = dr.RasterizeCudaContext()
    toc("init.RasterizeCudaContext", t_glctx)

    print("=" * 90)
    print("[INIT] 读取候选 OBJ 并构建 FoundationPose estimators")
    print("=" * 90)

    t_build_all_estimators = tic()
    candidates = []
    ...
    toc("init.build_all_estimators", t_build_all_estimators)

    if args.use_render_cache:
        print("=" * 90)
        print("[CACHE] 读取离线预渲染模板库")
        print("=" * 90)
        t_cache_load = tic()
        candidates = load_caches_for_candidates(candidates, args.render_cache_dir)
        toc("init.load_render_caches", t_cache_load)
    ...
    toc("init.YcbineoatReader", t_reader)
    ...
    for i in range(len(reader.color_files)):
        ...
        toc(f"frame.read_color | frame={frame_id}", t_read_color)

        t_read_depth = tic()
        depth = reader.get_depth(i)
        toc(f"frame.read_depth | frame={frame_id}", t_read_depth)

        pose = None
        did_track_branch = False

        if i == 0 or active is None:
            t_initial_branch = tic()
            mask = get_mask_safe(
                reader, i,
            ...
            toc(f"frame.initial_register_branch.total | frame={frame_id}", t_initial_branch)

        else:
            did_track_branch = True
            t_track_branch = tic()
            ...
            try:
                ...
                toc(f"track_one | {active['name']} | frame={frame_id}", t_track_one)
                ...
                toc(f"track.score_candidate_total | {active['name']} | frame={frame_id}", t_track_score)
        ...
        if did_track_branch:
            toc(f"frame.track_branch.total | frame={frame_id}", t_track_branch)
        ...
        toc(f"frame.total | frame={frame_id}", t_frame_total)
    ...
    toc("main.save_selected_object_history", t_history_save)
